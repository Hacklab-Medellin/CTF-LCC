...
if ($_POST["deleteconf"] && $_POST["field"]){
	...
	$query = "delete from custom_textbox_values where field_id = " . $_POST["field"];
	$db->query($query);
...
if ($_POST["submit"]){
	...
	if ($field_id)
	    $query = "select * from custom_textbox tb, custom_textarea ta, custom_dropdown dd where ((tb.template_var = '" . mysql_escape_string($_POST["template_var"]) . "'" . str_replace("cat_id", "tb.cat_id", $special_cats) . " and tb.id != '" . $_POST["field"] . "') or (ta.template_var = '" . mysql_escape_string($_POST["template_var"]) . "'" . str_replace("cat_id", "ta.cat_id", $special_cats) . " and ta.id != '" . $_POST["field"] . "') or (dd.template_var = '" . mysql_escape_string($_POST["template_var"]) . "'" . str_replace("cat_id", "dd.cat_id", $special_cats) . " and tb.id != '" . $_POST["field"] . "'))";
	else
		$query = "select * from custom_textbox tb, custom_textarea ta, custom_dropdown dd where ((tb.template_var = '" . mysql_escape_string($_POST["template_var"]) . "'" . str_replace("cat_id", "tb.cat_id", $special_cats) . ") or (ta.template_var = '" . mysql_escape_string($_POST["template_var"]) . "'" . str_replace("cat_id", "ta.cat_id", $special_cats) . ") or (dd.template_var = '" . mysql_escape_string($_POST["template_var"]) . "'" . str_replace("cat_id", "dd.cat_id", $special_cats) . ")) and tb.id != '" . $_POST["field"] . "'";
	$db->query($query);
	if ($db->next_record()){
		...
		if (!$_POST["field"] && !$error){
			$query = "insert into custom_textbox (`cat_id`, `name`, `template_var`, `description`, `searchable`) values ('" . mysql_escape_string($_POST["cat"]) . "', '" . mysql_escape_string($_POST["name"]) . "', '" . mysql_escape_string($_POST["template_var"]) . "', '" . mysql_escape_string($_POST["description"]) . "', '" . mysql_escape_string($_POST["searchable"]) . "')";
		    ...
		    $query = "update custom_textbox set `name` = '" . mysql_escape_string($_POST["name"]) . "', `template_var` = '" . mysql_escape_string($_POST["template_var"]) . "', `description` = '" . mysql_escape_string($_POST["description"]) . "', `searchable` = '" . mysql_escape_string($_POST["searchable"]) . "' where `id` = " . $_POST["field"];
		    $_POST["name"] = "";
			$_POST["template_var"] = "";
			$_POST["description"] = "";
			$_POST["cat"] = "";
    ...
    if ($cats == "()")
    	$query = "select * from custom_textbox where id=0";
    else
		$query = "select * from custom_textbox where $cats";
 	$db->query($query);
    while($db->next_record()){
		$inherited .= "<a class=ItechClsDataLink href=\"customtb.php?cat=" . $db->f("cat_id") . "&field=" . $db->f("id") . "\">&nbsp;&nbsp;&nbsp;" . $db->f("name") . "  ---Edit</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a class=ItechClsDataLink href=\"customtb.php?cat=" . $db->f("cat_id") . "&field=" . $db->f("id") . "&action=delete\">---!Delete!---</a><br>\n";
	}
	$query = "select * from custom_textbox where cat_id = '" . $CatID . "'";
	$db->query($query);
	while($db->next_record()){
		$thiscat .= "<a class=ItechClsDataLink href=\"customtb.php?cat=" . $db->f("cat_id") . "&field=" . $db->f("id") . "\">&nbsp;&nbsp;&nbsp;" . $db->f("name") . "  ---Edit</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a class=ItechClsDataLink href=\"customtb.php?cat=" . $db->f("cat_id") . "&field=" . $db->f("id") . "&action=delete\">---!Delete!---</a><br>\n";
	}
	if ($inherited)
		$inherited = "<font class=ItechClsFormHeaderFont>Inherited Custom Text Box Fields</font><br><br>\n" . $inherited;
	else
	    $inherited = "<font class=ItechClsFormHeaderFont>Inherited Custom Text Box Fields</font><br><br>\n" . "<a class=ItechClsDataLink href=\"javascript:void(0);\">&nbsp;&nbsp;&nbsp;None<br>";
	if ($thiscat)
		$thiscat = "<br><a class=ItechClsDataLink href=\"customtb.php?cat=$cat\"><font class=ItechClsFormHeaderFont>This Category's Custom Text Box Fields</font>  ---Add New</a><br><br>\n" . $thiscat;
	else
...
$FileName = "customtb.php";
$Redirect = "";
$TemplateFileName = "Themes/customtb.html";
$BlockToParse = "main";
$PathToRoot = "./";
//End Initialize Page
